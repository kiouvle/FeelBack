...
document.querySelector('.panel_colleagues').insertAdjacentHTML('beforeend', dictionary.colleagues.map((colleague, index) => new Colleague(colleague, index).create()).join(''));
...
document.querySelector('.panel_colleagues').addEventListener('click', (e, target) => popup.open(dictionary.colleagues[e.target.closest('.card').dataset.index]))
